...
## Current foundation (as of ~0.3.4)

Already shipping (as of ~0.3.4):
...
- Concept tags update `concept_mastery` on answers; Test my knowledge is a 10-question session with a strengths/gaps recap (session-scoped, not a full map)
...
- Room still reserves tasks/provider sync; `concept_mastery` is a thin per-tag score, not a syllabus
...
- No concept include-lists / Know-Learning-Not-yet UI; `concept_mastery` is a thin tag score, not a syllabus
...
Depends on Phase A–B data quality (history + concept scope). **0.3.2 C1 edge pick. 0.3.3 named stages. 0.3.4 Test my knowledge + silent concept scores.** Include-list / boundary chips still later.
